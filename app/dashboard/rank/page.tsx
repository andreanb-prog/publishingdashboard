...
export default function RankPage() {
  ...
  async function logRank(book: BookRecord, category: string | null) {
    const key = book.asin || book.id
  ...
  async function logAll(book: BookRecord) {
    const key = book.asin || book.id
    const iKey = inputKey(key, null)
    if (inputs[iKey]) await logRank(book, null)
  }
                ...
                <div key={key} className="card p-6">
                  ...
                  <div className="flex items-start justify-between mb-4">
                    <div>
                      ...
                      {book.asin && (
                        <div className="text-[10.5px] text-stone-500 font-mono ml-5.5">{book.asin}</div>
                      )}
                    </div>
                    {latestOverall !== null && (
                  ...
                  {/* Overall BSR input row */}
                  <div className="space-y-2.5 mb-4">
                    <div className="flex items-center gap-3">
                      <div className="w-44 flex-shrink-0">
                        <div className="text-[11.5px] font-semibold text-stone-600 truncate">Overall BSR</div>
                        {latestOverall !== null && (
                          <div className="text-[10.5px] font-mono" style={{ color: getRankColor(latestOverall) }}>
                            Last: #{latestOverall.toLocaleString()}
                          </div>
                        )}
                      </div>
                      <input
                        ...
                        onKeyDown={e => e.key === 'Enter' && logRank(book, null)}
                        className="input-field flex-1"
                        style={{ maxWidth: 160 }}
                      />
                      <button
                        onClick={() => logRank(book, null)}
                      ...
                      {overallLogs.length >= 2 && (
                        <div className="w-20 flex-shrink-0">
                          <Sparkline
                            points={[...overallLogs].reverse().slice(-14).map(l => l.rank)}
                            color={getRankColor(overallLogs[0].rank)}
                          />
                        </div>
                      )}

                      {latestOverall !== null && latestOverall < 100 && (
                  ...
                  <div className="flex items-center gap-3 border-t border-stone-100 pt-4">
                    ...
                    {!book.asin && (
                      <div className="text-[12px] text-stone-500 flex items-center gap-1.5">
                        <span>💡 No ASIN set.</span>
                        <Link href="/dashboard/settings#my-books" className="text-amber-600 font-semibold hover:underline">
                          Add it in Settings →
                        </Link>
                      </div>
                    )}
