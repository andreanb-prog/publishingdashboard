...
export async function POST(req: NextRequest) {
  ...
  if (hasOnlyZeroOrNullNumbers(body)) {
    await db.extensionSyncLog.create({
      data: { userId: user.id, platform: 'meta', dataPoints: 0, status: 'rejected_zeros' },
    })
  ...
  await db.extensionSyncLog.create({
    data: { userId: user.id, platform: 'meta', dataPoints, status: 'success' },
  })

  // TODO: persist Meta data to MetaAdData / MetaAdStat tables
