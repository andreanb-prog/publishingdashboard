...
- MailerLite API v3 ONLY: use `connect.mailerlite.com/api` with `Authorization: Bearer {key}` header. Subscriber counts: `GET /subscribers/stats` → `{ total, active, unsubscribed, ... }`. The cursor-paginated `/subscribers` endpoint does NOT include `meta.total`. NEVER use `api.mailerlite.com/api/v2`, `/api/v2/stats`, or `X-MailerLite-ApiKey` header.
